...
// XXX-albarc Para poner trazas con un collectTraceInfo diferente
std::ostream & CastaliaModule::traceAlbarc()
{
	if (hasPar("collectTraceInfoAlbarc") && par("collectTraceInfoAlbarc")) {
		return (ostream &) DebugInfoWriter::getStream() <<
		  "\n" << setw(16) << simTime() << " " << setw(40) << getFullPath() << " ";
	} else {
		return empty;
