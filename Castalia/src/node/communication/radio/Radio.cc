...
void Radio::handleMessage(cMessage * msg)
{
	...
	switch (msg->getKind()) {
		...
		case WC_SIGNAL_START:{
			...
			traceAlbarc() << "START signal from node " << wcMsg->getNodeID() << " , received power " << wcMsg->getPower_dBm() << "dBm" ;
			...
			if ((state != RX) || (changingToState != -1)) {
				...
				traceAlbarc() << "Failed packet (WC_SIGNAL_START) from node " << newSignal.ID << ", radio not in RX state";
			...
			if ((RXmode->modulation == newSignal.modulation) && (newSignal.power_dBm >= RXmode->sensitivity))
				...
				if (newSignal.power_dBm < RXmode->sensitivity) {
					...
					traceAlbarc() << "Failed packet (WC_SIGNAL_START) from node " << newSignal.ID << ", below sensitivity";
				}
				else {
					stats.RxFailedModulation++;
					trace() << "Failed packet (WC_SIGNAL_START) from node " << newSignal.ID << ", wrong modulation";
					traceAlbarc() << "Failed packet (WC_SIGNAL_START) from node " << newSignal.ID << ", wrong modulation";
				}
			}
		...
		case WC_SIGNAL_END:{
			...
			if ((state != RX) || (changingToState != -1)) {
				if (endingSignal->bitErrors != ALL_ERRORS) {
					...
					traceAlbarc() << "Failed packet (WC_SIGNAL_END) from node " << signalID << ", no RX state";
				}
			...
			if (endingSignal->bitErrors != ALL_ERRORS) {
				if (endingSignal->bitErrors <= maxErrorsAllowed(endingSignal->encoding)) {
					...
					if (endingSignal->maxInterference == RXmode->noiseFloor) {
						...
						traceAlbarc() << "Received packet (WC_SIGNAL_END) from node " << signalID << ", with no interference";
					}
					else {
						stats.RxReachedInterference++;
						trace() << "Received packet (WC_SIGNAL_END) from node " << signalID << ", despite interference";
						traceAlbarc() << "Received packet (WC_SIGNAL_END) from node " << signalID << ", despite interference";
					}
				} else {
					// collect stats
					if (endingSignal->maxInterference == RXmode->noiseFloor) {
						...
						traceAlbarc() << "Failed packet (WC_SIGNAL_END) from node " << signalID << ", NO interference";
					}
					else {
						stats.RxFailedInterference++;
						trace() << "Failed packet (WC_SIGNAL_END) from node " << signalID << ", with interference";
						traceAlbarc() << "Failed packet (WC_SIGNAL_END) from node " << signalID << ", with interference";
					}
				}
			}
		...
		case MAC_LAYER_PACKET:{
			...
			traceAlbarc() << "Nodo " << self << " RECIBIDO MAC LAYER PACKET";
			...
			if (maxPhyFrameSize != 0 && totalSize > maxPhyFrameSize) {
				...
				traceAlbarc() << "Nodo " << self << " oversized MAC LAYER PACKET dropped";
				break;
			}

			if ((int)radioBuffer.size() < bufferSize) {
				trace() << "Buffered [" << macPkt->getName() << "] from MAC layer";
				traceAlbarc() << "Nodo " << self << ", Buffered [" << macPkt->getName() << "] from MAC layer";
				...
				traceAlbarc() << "Nodo " << self << " FULL buffer, MAC LAYER PACKET dropped";
			}
			break;
		}
...
void Radio::handleRadioControlCommand(RadioControlCommand * radioCmd)
{
	switch (radioCmd->getRadioControlCommandKind()) {
		...
		case SET_STATE:{
			...
			traceAlbarc() << "SET STATE to " << (changingToState == TX ? "TX" :
					(changingToState == RX ? "RX" : "SLEEP")) << ", delay=" <<
					transitionDelay << ", power=" << avgDrawnTransitionPower;
...
double Radio::popAndSendToWirelessChannel()
{
	...
	traceAlbarc() << "Nodo " << self << ", Sending Packet, Transmission will last " << txTime << " secs";
	return txTime;
}
